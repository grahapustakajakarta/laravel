...
@section('title', 'Manajemen MAGZ')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="page-title">MAGZ</h2>
        <p class="text-muted mb-0" style="font-size:0.9rem;">Kelola dokumen majalah yang dapat dibeli/diunduh pengunjung.</p>
    </div>
    <a href="{{ route('admin.magz.create') }}" class="btn btn-primary"><i class="fas fa-plus me-1"></i> Tambah MAGZ</a>
</div>

{{-- Filter --}}
<div class="card shadow-sm mb-4">
    <div class="card-body py-3">
        <form method="GET" action="{{ route('admin.magz.index') }}" class="d-flex align-items-center gap-3">
            ...
            @if(request('kategori'))
                <a href="{{ route('admin.magz.index') }}" class="btn btn-sm btn-outline-secondary">Reset</a>
            @endif
        </form>
    </div>
</div>
...
<div class="card shadow-sm">
    <div class="card-body">
        <table class="table table-hover w-100 align-middle" id="tbl-magz">
            <thead>
                <tr>
                    <th width="5%">#</th>
            ...
            <tbody>
                @forelse($magzs as $index => $item)
                <tr>
                    <td>{{ $magzs->firstItem() + $index }}</td>
                    <td>
                    ...
                    <td class="text-muted" style="font-size:0.9rem;">{{ $item->penulis ?: '-' }}</td>
                    ...
                    <td>
                        <a href="{{ route('admin.magz.edit', $item->id) }}" class="btn btn-sm btn-outline-dark"><i class="fas fa-pen-to-square"></i> Edit</a>
                        <form action="{{ route('admin.magz.destroy', $item->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
