...
<style>
    ...
    #pustaka-beli-scope .pustaka-container {
        max-width: 1000px;
        margin: 40px auto;
        padding: 0 30px;
        width: 100%;
    ...
    #pustaka-beli-scope .breadcrumb a {
        color: #000;
        text-decoration: none;
        transition: color 0.2s;
    }
    #pustaka-beli-scope .breadcrumb a:hover {
        color: #e31837;
    }

    #pustaka-beli-scope .breadcrumb span {
        color: #555 !important;
    }

    /* --- TATA LETAK UTAMA (2 KOLOM) --- */
    #pustaka-beli-scope .product-layout {
        display: flex;
        gap: 60px;
    ...
    #pustaka-beli-scope .product-image-container {
        flex: 0 0 340px;
    ...
    #pustaka-beli-scope .product-image {
        width: 100%;
        height: auto;
    ...
    /* Kolom Kanan: Detail */
    #pustaka-beli-scope .product-details {
        flex: 1;
        padding-top: 10px;
    }

    #pustaka-beli-scope .product-title {
    ...
    #pustaka-beli-scope .product-author {
        font-family: "Open Sans", sans-serif !important;
    ...
    /* Harga & SKU */
    #pustaka-beli-scope .price-sku-row {
        display: flex;
    ...
    #pustaka-beli-scope .product-price {
        font-family: "Open Sans", sans-serif !important;
        font-size: 13px !important;
        font-weight: 600 !important;
        letter-spacing: 0.5px;
    ...
    #pustaka-beli-scope .btn-bayar:hover {
    ...
    /* Deskripsi */
    #pustaka-beli-scope .product-description {
        font-family: "Crimson Text", serif !important;
    ...
    #pustaka-beli-scope .product-description br + br {
        display: block;
        content: "";
        margin-top: -10px;
    }
    ...
    #pustaka-beli-scope .recommendation-section {
        margin-top: 80px;
    ...
    #pustaka-beli-scope .section-title {
        font-family: "Crimson Text", serif !important;
        font-size: 24px;
        color: #111;
        text-align: center;
    ...
    #pustaka-beli-scope .carousel-wrapper {
        position: relative;
    }

    #pustaka-beli-scope .carousel {
        display: flex;
        gap: 24px;
        overflow-x: auto;
        padding: 10px 5px;
    ...
    #pustaka-beli-scope .carousel::-webkit-scrollbar {
        display: none;
    }

    #pustaka-beli-scope .book-card {
    ...
    #pustaka-beli-scope .book-card:hover {
        transform: translateY(-5px);
    }
    
    /* Hapus juga background bawaan dari gambar carousel jika ada */
    #pustaka-beli-scope .book-card-image {
        width: 100%;
        aspect-ratio: 2/3;
        margin-bottom: 16px;
        background: transparent !important;
    }
    #pustaka-beli-scope .book-card-image img {
        width: 100%;
        height: 100%;
        object-fit: cover;
    ...
    #pustaka-beli-scope .book-card-author {
        font-family: "Open Sans", sans-serif !important;
        font-size: 0.8rem;
        color: #777;
        text-transform: uppercase;
    ...
    #pustaka-beli-scope .book-card-title {
        font-family: "Crimson Text", serif !important;
    ...
    #pustaka-beli-scope .book-card-price {
        font-family: "Open Sans", sans-serif !important;
        font-weight: 600;
        color: #111;
        font-size: 0.95rem;
    ...
    /* Navigation Buttons */
    #pustaka-beli-scope .carousel-nav-left,
    #pustaka-beli-scope .carousel-nav-right {
        position: absolute;
        top: 50%;
    ...
    #pustaka-beli-scope .carousel-nav-left:hover,
    #pustaka-beli-scope .carousel-nav-right:hover {
        background-color: #f0f0f0;
    }
    #pustaka-beli-scope .carousel-nav-left { left: -15px; }
    #pustaka-beli-scope .carousel-nav-right { right: -15px; }
...
<div id="pustaka-beli-scope">
    <div class="pustaka-container">
      <nav class="breadcrumb">
        <a href="{{ url('/') }}">HOME</a> / <a href="{{ route('pustaka.index') }}">Katalog Pustaka</a> / <span>{{ strtoupper($pustaka->judul) }}</span>
      </nav>
      ...
      <div class="product-layout">
        <div class="product-image-container">
          @if($pustaka->gambar_1)
          <img
            src="{{ asset('img/' . $pustaka->gambar_1) }}"
            alt="{{ $pustaka->judul }} Cover"
            class="product-image"
          />
          @else
          <img
            src="https://picsum.photos/seed/{{ $pustaka->id }}/400/550"
            alt="{{ $pustaka->judul }} Cover"
            class="product-image"
          />
          @endif
        ...
        <div class="product-details">
          <h1 class="product-title">{{ $pustaka->judul }}</h1>
          @if($pustaka->penulis)
              <div class="product-author">By {{ $pustaka->penulis->nama ?? 'Unknown' }}</div>
          @endif

          <div class="price-sku-row">
            @if($pustaka->harga > 0)
            <div class="product-price">Rp {{ number_format($pustaka->harga, 0, ',', '.') }}</div>
            @else
          ...
          @if(isset($hasAccess) && $hasAccess)
              <a href="{{ route('pustaka.baca', $pustaka->slug) }}" class="btn-bayar" style="margin-bottom: 15px; display: inline-block; text-align: center; text-decoration: none; background-color: #111; color: #fff; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#333'" onmouseout="this.style.backgroundColor='#111'">UNDUH <i class="fas fa-arrow-down" style="font-size: 0.9rem; margin-left: 5px;"></i></a>
          ...
          @else
              <div style="display: flex; gap: 12px; align-items: stretch; margin-bottom: 15px;">
                  <form action="{{ route('pustaka.bayar', $pustaka->slug) }}" method="POST" style="margin: 0;">
                      @csrf
          ...
          <div class="product-description">
            @foreach(explode("\n", str_replace("\r", "", $pustaka->deskripsi ?? 'Belum ada deskripsi.')) as $paragraph)
                @if(trim($paragraph) !== '')
      ...
      <section class="recommendation-section">
          ...
          <div class="carousel-wrapper">
              ...
              <div class="carousel" id="recCarousel">
                  ...
                  <div class="book-card" onclick="window.location='{{ route('pustaka.detail', $rec->slug) }}'">
                      ...
                      <p class="book-card-author">{{ $rec->kategori ?? 'Pustaka' }}</p>
    ...
    document.getElementById('pay-button').onclick = function(){
        snap.pay('{{ $snapToken }}', {
            onSuccess: function(result){
                ...
                window.location.href = "{{ route('pustaka.beli', $pustaka->slug) }}?order_id=" + result.order_id + "&transaction_status=" + result.transaction_status;
            },
